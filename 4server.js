...
async function findWorkingModel() {
    ...
    try {
        ...
        // Try the most common working models directly first
        const modelsToTry = [
            'gemini-1.5-pro',
            'gemini-1.5-flash',
            'gemini-1.0-pro',
        ...
        if (data.models && Array.isArray(data.models)) {
            ...
            if (workingModels.length > 0) {
                ...
                console.log(`\n🎯 Selected model from list: ${selectedModel}`);
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        res.json({
            ...
            emotion: emotion,
            hasImage: !!imageData,
            sources: [] // Optional: add grounding sources if needed
        });
...
app.post('/api/tts', async (req, res) => {
    try {
        ...
        // The frontend will use browser TTS as fallback
        res.json({
            success: true,
            audioData: null,
            mimeType: 'audio/wav'
...
app.get('/api/test', async (req, res) => {
    try {
        ...
        const testUrl = API_LLM_URL || `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent`;
        
        const response = await fetch(`${testUrl}?key=${cleanApiKey}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(testPayload)
        });
